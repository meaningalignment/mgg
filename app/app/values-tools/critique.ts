...
import type { DeduplicatedCard } from "@prisma/client"
...
export async function updateCardFromForm(formData: FormData) {
  ...
  if (cardType === "canonical") {
    await db.deduplicatedCard.update({ where: { id: cardId }, data })
...
export async function runTaskFromForm(formData: FormData) {
  ...
  if (task === 'regenerateInstructionsDetailed') {
    ...
    const card = await db.deduplicatedCard.findUnique({ where: { id: Number(formData.get("cardId")) } })
    await embeddingService.embedDeduplicatedCard(card as any as DeduplicatedCard)
    return json({ ok: true })
  } else {
