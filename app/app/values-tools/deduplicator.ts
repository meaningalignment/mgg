import type { DeduplicatedCard, Prisma, ValuesCard } from "@prisma/client"
import { embeddingService as embeddings } from "./embedding"
import type { ChatCompletionFunctions } from "openai-edge"
import { cardModel, db, inngest, openai } from "~/config.server"
import { attentionPoliciesCriteria } from "./prompt-segments"
import { DBSCAN } from "density-clustering"
import { ValuesCardModel } from "./value-model"

const model = "gpt-4-1106-preview"
const generation = 3 // This is dynamic in mgg.
...
function toDataModelArrayWithId(cards: ValuesCard[] | DeduplicatedCard[]) {
  return cards.map((card) => ({ id: card.id, policies: card.policies }))
}

function toDataModelArrayWithIdJSON(
  cards: ValuesCardModel[]
): string {
  return JSON.stringify(
    cards.map((card) => ({
      id: card.id,
      attentionPolicies: 'evaluationCriteria' in card ? card.evaluationCriteria : card.policies,
    }))
  )
}
...
async function createDeduplicatedCard(
  data: ValuesCardModel,
  generation: number
) {
 ...
 */
async function getBestValuesCard(card_ids: number[]) {
 ...
 */
async function fetchSimilarDeduplicatedCard(
  ...
  const deduplicated = await similaritySearch(card_embeddings, limit, 0.1)
...
export const seedGeneration = inngest.createFunction(
  ...
  async ({ step, logger }) => {
    let generation: number = 3
    if (cardModel == "mgg") {
      generation = (await step.run(
      `Create a new deduplication generation`,
      async () => {
        // @ts-ignore: Schema don't match.
        return db.deduplicationGeneration.create({
          data: { gitCommitHash: process.env.VERCEL_GIT_COMMIT_SHA || "dev" },
        })
      }
    )).id

    const clusters = await step.run(`Clustering using dbscan`, async () => {
    ...
    for (const cluster of clusters) {
      if (cluster.length < 2) {
        ...
        const representative = (await step.run(
          ...
          async () => getBestValuesCard(cluster)
        )) as any as ValuesCard

        (await step.run(
          "Add representative to deduped generation, and embed",
          async () => createDeduplicatedCard(representative, generation)
  ...
  async ({ step, logger }) => {
    ...
    const cards = (await step.run(
      ...
      async () => {
        return await db.valuesCard.findMany({
          take: 100, 
          where: { Deduplication: { none: generation } }
        }))       as any as ValuesCard[]
    ...
    for (const card of cards) {
      ...
      const existingCanonicalDuplicate = (await step.run(
        ...
        async () => fetchSimilarDeduplicatedCard(card)
      ...
      if (existingCanonicalDuplicate) {
        await step.run(
          ...
          async () => {
            // @ts-ignore: Schema don't match.
            const table = cardModel == "mgg" ? db.valuesCardToDeduplicatedCard : db.deduplication
            await table.create({
              data: {
                generation,
