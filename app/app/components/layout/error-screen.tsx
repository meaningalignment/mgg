...
export default function LoadingScreen({ title, subtitle }: Props) {
  ...
  return (
    <div className="flex flex-col h-screen w-screen mt-12 md:mt-24">
      <div className="flex flex-col items-center space-y-6 mx-4 md:mx-8">
        <AlertCircle className={fadeIn(show, 0, "text-red-500 h-8 w-8 mb-3")} />
        {title && <h1 className={fadeIn(show, 75, "text-xl md:text-2xl font-semibold text-center")}>{title}</h1>}
        {subtitle && <h2 className={fadeIn(show, 125, "text-sm md:text-base text-gray-700 text-center")}>{subtitle}</h2>}
      </div>
    </div>
  )
}